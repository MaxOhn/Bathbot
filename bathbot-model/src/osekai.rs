...
define_ranking! {
    RankedMapsets { "mapsets", "ranked" }
}

define_ranking! {
    LovedMapsets { "mapsets", "loved" }
}
...
mod compact {
        ...
        T: de::Deserialize<'de>,
    {
        ...
        impl<'de, T: de::Deserialize<'de>> de::Visitor<'de> for CompactVisitor<T> {
            ...
            // osekai responds with an empty array if there are no items
            fn visit_seq<A: de::SeqAccess<'de>>(self, mut seq: A) -> Result<Self::Value, A::Error> {
        ...
        d.deserialize_any(CompactVisitor(PhantomData))
    }
}
...
mod tests {
    ...
    #[test]
    fn deserialize_empty_ranking() {
        const JSON: &str = r#"{"_dev":"Compressed response. Remove 'compress' parameter for human-readable form.","success":true,"message":"ok","content":{"data":[],"max":10000}}"#;

        let OsekaiInex {
            content: OsekaiRankingEntries::<OsekaiRankingEntry> { data, .. },
    ...
    fn deserialize_rarity_ranking() {
        ...
        let OsekaiInex {
            content: OsekaiRankingEntries::<OsekaiRarityEntry> { data, .. },
        } = serde_json::from_str(JSON).unwrap();
