...
impl<'m> MyOsuPP<'m> {
    pub fn calculate_v2018(mut self) -> PpResult {
        ...
        let aim_value = self.compute_aim_value_v2018(total_hits as f32);
        let speed_value = self.compute_speed_value_v2018(total_hits as f32);
        let acc_value = self.compute_accuracy_value_v2018(total_hits);
    ...
    fn compute_aim_value_v2018(&self, total_hits: f32) -> f32 {
    ...
    fn compute_speed_value_v2018(&self, total_hits: f32) -> f32 {
        let attributes = self.attributes.as_ref().unwrap();
    ...
    fn compute_accuracy_value_v2018(&self, total_hits: usize) -> f32 {
        let attributes = self.attributes.as_ref().unwrap();
