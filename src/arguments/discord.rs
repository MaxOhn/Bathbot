...
impl RoleAssignArgs {
    pub fn new(mut args: Args) -> Result<Self, String> {
        let args = arguments::first_n(&mut args, 3);
        let mut iter = args.into_iter();
        let rgx = Regex::new(r"<#([0-9]*)>$").unwrap();
        let channel_id = iter.next().and_then(|arg| parse(&arg, &rgx)).map(ChannelId);
        ...
        let rgx = Regex::new(r"<@&([0-9]*)>$").unwrap();
        let role_id = iter.next().and_then(|arg| parse(&arg, &rgx)).map(RoleId);
        if role_id.is_none() {
...
impl RoleArgs {
    pub fn new(mut args: Args) -> Self {
        ...
        let rgx = Regex::new(r"<@&([0-9]*)>$").unwrap();
        let role_id = args.pop().and_then(|arg| parse(&arg, &rgx)).map(RoleId);
        Self { role_id }
    }
}
