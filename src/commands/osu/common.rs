            ...
            let map = match req.queue_single(&osu).await {
                Ok(result) => match result {
                    ...
                    None => {
                        ...
                        bail!("API returned no result for map {}", id);
                    }
                },
                Err(why) => {
                    msg.respond(&ctx, OSU_API_ISSUE).await?;
...
pub async fn common(ctx: Arc<Context>, msg: &Message) -> BotResult<()> {
...
#[example("badewanne3 \"nathan on osu\" idke")]
#[aliases("commonm")]
pub async fn commonmania(ctx: Arc<Context>, msg: &Message) -> BotResult<()> {
...
#[example("badewanne3 \"nathan on osu\" idke")]
#[aliases("commont")]
pub async fn commontaiko(ctx: &Context, msg: &Message, args: Args) -> BotResult<()> {
...
#[example("badewanne3 \"nathan on osu\" idke")]
#[aliases("commonc")]
pub async fn commonctb(ctx: Arc<Context>, msg: &Message) -> BotResult<()> {
