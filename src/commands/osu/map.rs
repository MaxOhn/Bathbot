use crate::{
    arguments::{MapModArgs, ID},
    embeds::{EmbedData, MapEmbed},
    pagination::{MapPagination, Pagination},
    roppai::Oppai,
    util::{discord, globals::OSU_API_ISSUE, osu::prepare_beatmap_file, MessageExt},
    MySQL, Osu,
};
...
#[command]
#[description = "Display stats about a beatmap. Mods can be specified.\n\
...
async fn map(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    ...
    let (maps, first_map_id) = {
        ...
        if maps.is_empty() {
            msg.channel_id
                .say(ctx, "API returned no map for this id")
                .await?
                .reaction_delete(ctx, msg.author.id)
                .await;
            return Ok(());
    ...
    .await
    {
        ...
        Err(why) => {
            msg.channel_id
                .say(ctx, "Some issue while calculating map data, blame bade")
                .await?
                .reaction_delete(ctx, msg.author.id)
                .await;
            return Err(why.into());
